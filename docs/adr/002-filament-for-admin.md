...
- `php artisan filament:install --panels` — создаёт `app/Providers/Filament/AdminPanelProvider.php`
- `composer require spatie/laravel-permission:^6.4`, публикация конфига + миграция с префиксом `spatie_*`

Sync доменных ролей со Spatie — см. [ADR-010: Spatie Permission с доменными ролями](010-spatie-permission-with-domain-roles.md).
