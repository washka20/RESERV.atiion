...
describe('Pagination', () => {
  it('disables Prev on first page', () => {
    ...
    const prev = wrapper.get('[data-test-id="base-pagination-prev"]')
    const next = wrapper.get('[data-test-id="base-pagination-next"]')
  ...
  it('emits change with previous page on Prev click', async () => {
    ...
    await wrapper.get('[data-test-id="base-pagination-prev"]').trigger('click')
    expect(wrapper.emitted('change')).toEqual([[2]])
  })

  it('emits change with next page on Next click', async () => {
    const wrapper = mount(Pagination, { props: { meta: meta(3, 5) } })
    await wrapper.get('[data-test-id="base-pagination-next"]').trigger('click')
